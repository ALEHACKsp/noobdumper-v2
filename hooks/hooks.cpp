...
tNtWriteVirtualMemory oNtWriteVirtualMemory = NULL;
...
)
{
    ...
    if ( ( ( char* ) pBuffer )[ 0 ] == 0x4D && ( ( char* ) pBuffer )[ 1 ] == 0x5A )
        //if ( ( ( DWORD* ) pBuffer )[ 0 ] == 0x4D5A )
        mzString = "<- MZ Sig Found";
    else
        mzString = "None";

    printf( "NtWriteVirtualMemory ( 0x%p, 0x%p, 0x%p, 0x%d, 0x%p ) %s \r\n", hProcess, ( PVOID ) &pBaseAddress, ( PVOID ) &pBuffer, uNumberOfBytesToWrite, ( PVOID ) &puNumberOfBytesWritten, mzString );
    ...
    BOOL bSuccess = WriteFile( hFile, pBuffer, uNumberOfBytesToWrite, &dwBytes, 0 );
    if ( !bSuccess )
    ...
    return oNtWriteVirtualMemory( hProcess, pBaseAddress, pBuffer, uNumberOfBytesToWrite, puNumberOfBytesWritten );
}

//doesn't really matter if NtAllocateVirtualMemory is the one called
//NtWriteVirtualMemory is just enough for the dump.
LPVOID
WINAPI
hkVirtualAllocEx
(
